...
const sortedItems = computed(() => {
  return [...fridgeItems.value].sort((a, b) => {
...
const activeItemCount = computed(() => {
  return fridgeItems.value.filter((item) => !item.done).length
})
...
async function removeIngredient(item) {
  ...
  fridgeItems.value = fridgeItems.value.filter((target) => target.id !== item.id)
...
<template>
  <section class="fridge-view">
    ...
    <div v-if="loading" class="loading">냉장고 문 여는 중...</div>

    <div v-else-if="sortedItems.length === 0" class="empty">냉장고가 비어 있어요.</div>
    ...
    <div v-else class="ingredient-grid">
      ...
      >
        ...
        <footer class="card-footer">
          <span class="storage-badge"> 냉장 </span>
        </footer>
      </article>
    </div>

    <form class="add-form" @submit.prevent="addIngredient">
      <input v-model="ingredientName" type="text" maxlength="40" placeholder="예: 삼겹살 500g" />
...
.add-form input:focus {
  border-color: rgb(255 158 177 / 48%);
  outline: 3px solid rgb(255 158 177 / 12%);
}
